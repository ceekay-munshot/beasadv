...
//
// Primary provider is chosen by env LLM_PROVIDER ('claude' default); the others
// (gemini, mistral) are fallbacks. Each gets one retry on invalid/schema JSON.
...
// Mistral: prefer the newer key if present, else the original.
const mistralKey = () => process.env.MISTRAL_API_KEY_NEW || process.env.MISTRAL_API_KEY;
// 'gemini-flash-latest' is Google's rolling alias to the current free flash
// model, so this default won't rot when a pinned version is retired.
...
// Google Gemini (free tier). The API key goes in the query string per Google's
// API; the system prompt maps to systemInstruction and JSON mode is requested
// via responseMimeType. Loose-JSON parsing + schema validation happen upstream.
async function callGemini({ prompt, system }) {
  if (!hasGemini()) throw new Error('GEMINI_API_KEY not set');
  const url = `https://generativelanguage.googleapis.com/v1beta/models/${encodeURIComponent(geminiModel())}:generateContent?key=${process.env.GEMINI_API_KEY}`;
  const res = await fetch(url, {
    method: 'POST',
  ...
  // never surface the URL on error — it carries the key; only status + body snippet
...
const PROVIDERS = { claude: callBedrock, gemini: callGemini, mistral: callMistral };

function providerOrder() {
  // primary from LLM_PROVIDER (default claude), then the rest as fallbacks;
  // keep only providers that actually have a key configured.
  const all = ['claude', 'gemini', 'mistral'];
  const primary = (process.env.LLM_PROVIDER || 'claude').toLowerCase();
  const order = [primary, ...all.filter((p) => p !== primary)].filter((p) => all.includes(p));
...
export function providerAvailable(p) {
  return p === 'claude' ? hasClaude() : p === 'gemini' ? hasGemini() : p === 'mistral' ? hasMistral() : false;
}
...
export async function completeJSON({ prompt, system, schema }) {
  ...
  if (!order.length) throw new Error('no LLM provider configured (need CLAUDE_BEDROCK_API_KEY, GEMINI_API_KEY or MISTRAL_API_KEY)');
